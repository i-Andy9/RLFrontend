...
const RoomsList = ({ sesion }) => {
  const { rooms, getRoomsList, getRoom, deleteRoom, } = useContext(RoomContext);
  const { sesionActivity, token } = useContext(AdminContext);
  const getItem = (code) => {
    getRoom(code);
  };
  const deleteItemRoom = (id) => {
    deleteRoom(id, token)
  }
  useEffect(() => {
    getRoomsList();
                  ...
                  {rooms.length > 0 ? (
                    rooms.map((room, i) => (
                      <tr key={i} className="text-center">
                        <th>{i + 1}</th>
                        <td>{room.name}</td>
                        <td>{`${room.details.substr(0, 25)}...`}</td>
                        <td>{room.code}</td>
                        <td>
                          <button
                            className="p-1 hover:bg-gray-400 rounded-lg"
                            type="button"
                            onClick={() => getItem(room.code)}
                          >
                            <svg
                            ...
                            <>
                              ...
                              >
                                <svg
                                  ...
                                  viewBox="0 0 16 16"
                                  onClick={() => deleteItemRoom(room.code)}
                                >
